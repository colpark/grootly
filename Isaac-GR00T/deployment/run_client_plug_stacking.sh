#!/bin/bash
# GR00T Robot Client for Plug Stacking Task
#
...
#   ./deployment/run_client_plug_stacking.sh --dry-run    # Test without robot commands
#   ./deployment/run_client_plug_stacking.sh --mock       # Test without robot hardware
#   ./deployment/run_client_plug_stacking.sh --test-only  # Test server connection only
#
...
TASK_INSTRUCTION="${TASK_INSTRUCTION:-Stack the plugs}"
SERVER_IP="${SERVER_IP:-130.199.95.27}"
SERVER_PORT="${SERVER_PORT:-5561}"

# Robot hardware configuration (same as ball2 - same robot)
...
echo "=============================================="
echo "GR00T Client: Plug Stacking Task"
echo "=============================================="
echo "Task: ${TASK_NAME}"

#!/bin/bash
# Evaluate ball2_groot model at multiple checkpoint steps
#
# Usage:
#   ./finetuning/evaluate_checkpoints.sh 1000 2000 3000 4000 5000
#   ./finetuning/evaluate_checkpoints.sh --all  # Evaluate all checkpoints
#
# Or with custom output dir:
#   OUTPUT_DIR=./my_results ./finetuning/evaluate_checkpoints.sh 1000 2000

set -e

CHECKPOINT_DIR="${CHECKPOINT_DIR:-./outputs/ball2_groot}"
DATASET_PATH="${DATASET_PATH:-./data/ball2_groot_test}"
OUTPUT_BASE="${OUTPUT_DIR:-./eval_results/ball2_groot}"
...
if [[ -z "$STEPS" ]]; then
    ...
    echo "  CHECKPOINT_DIR  Directory containing checkpoints (default: ./outputs/ball2_groot)"
    echo "  DATASET_PATH    Path to test dataset (default: ./data/ball2_groot_test)"
    echo "  OUTPUT_DIR      Output directory for results (default: ./eval_results/ball2_groot)"
    exit 1
fi

echo "=================================================="
echo "Ball2 GR00T Checkpoint Evaluation"
echo "=================================================="

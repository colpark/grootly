#!/usr/bin/env python3
"""
Evaluation script for trossen_lego-finetuned GR00T model.

This script:
1. Loads the finetuned checkpoint
2. Runs open-loop evaluation on the lego test dataset
...
Robot Configuration (Trossen AI Mobile - same as ball2_groot and plug_stacking):
...
Usage:
    python finetuning/evaluate_lego.py \
        --checkpoint_path ./outputs/lego/checkpoint-1000 \
...
# Import modality config registration (same as ball2_groot and plug_stacking - Trossen AI Mobile)
...
# Action dimension labels for lego (Trossen AI Mobile bimanual robot)
...
# State dimension labels for lego
STATE_LABELS = [
    # Base odometry (3 DOF)
    "odom_x (m)",
    "odom_y (m)",
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate lego-finetuned GR00T model")
    parser.add_argument("--checkpoint_path", type=str, default=None,
                        help="Path to checkpoint (default: auto-detect latest)")
    parser.add_argument("--dataset_path", type=str, default="./data/lego_test",
                        help="Path to test dataset")
    parser.add_argument("--output_dir", type=str, default="./eval_results/lego",
    ...
    if checkpoint_path is None:
        checkpoint_path = find_latest_checkpoint("./outputs/lego")
        if checkpoint_path is None:
            logger.error("No checkpoint found in ./outputs/lego/. Please specify --checkpoint_path")
            sys.exit(1)
    ...
    if not Path(args.dataset_path).exists():
        ...
        logger.error("  1. python scripts/download_hf_dataset.py TrossenRoboticsCommunity/trossen_ai_mobile_lego --name trossen_lego")
        logger.error("  2. python finetuning/convert_lego.py")
        sys.exit(1)
    ...
    if all_mse:
        ...
        with open(summary_path, "w") as f:
            f.write("=" * 60 + "\n")
            f.write("LEGO Manipulation GR00T Evaluation Results\n")
            f.write("Trossen AI Mobile - LEGO Manipulation Task\n")
            f.write("=" * 60 + "\n\n")

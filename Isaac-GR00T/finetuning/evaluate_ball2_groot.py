#!/usr/bin/env python3
"""
Evaluation script for ball2_groot-finetuned GR00T model.

This script:
1. Loads the finetuned checkpoint
2. Runs open-loop evaluation on the ball2_groot test dataset
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate ball2_groot-finetuned GR00T model")
    parser.add_argument("--checkpoint_path", type=str, default=None,
                        help="Path to checkpoint (default: auto-detect latest)")
    parser.add_argument("--dataset_path", type=str, default="./data/ball2_groot_test",
                        help="Path to test dataset")
    parser.add_argument("--output_dir", type=str, default="./eval_results/ball2_groot",
    ...
    if checkpoint_path is None:
        checkpoint_path = find_latest_checkpoint("./outputs/ball2_groot")
        if checkpoint_path is None:
            logger.error("No checkpoint found in ./outputs/ball2_groot/. Please specify --checkpoint_path")
            sys.exit(1)
    ...
    if not Path(args.dataset_path).exists():
        ...
        logger.error("Please run: python finetuning/convert_ball2_groot.py")
        sys.exit(1)
    ...
    if all_mse:
        ...
        with open(summary_path, "w") as f:
            f.write("=" * 60 + "\n")
            f.write("Ball2 GR00T Evaluation Results\n")
            f.write("Trossen AI Mobile - Ball Transfer Task\n")
            f.write("=" * 60 + "\n\n")

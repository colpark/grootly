#!/bin/bash
# Evaluate lego model at multiple checkpoint steps
#
# Usage:
#   ./finetuning/evaluate_lego_checkpoints.sh 1000 2000 3000 4000 5000
#   ./finetuning/evaluate_lego_checkpoints.sh --all  # Evaluate all checkpoints
#
# Or with custom output dir:
#   OUTPUT_DIR=./my_results ./finetuning/evaluate_lego_checkpoints.sh 1000 2000

set -e

CHECKPOINT_DIR="${CHECKPOINT_DIR:-./outputs/lego}"
DATASET_PATH="${DATASET_PATH:-./data/lego_test}"
OUTPUT_BASE="${OUTPUT_DIR:-./eval_results/lego}"
...
if [[ -z "$STEPS" ]]; then
    ...
    echo "  CHECKPOINT_DIR  Directory containing checkpoints (default: ./outputs/lego)"
    echo "  DATASET_PATH    Path to test dataset (default: ./data/lego_test)"
    echo "  OUTPUT_DIR      Output directory for results (default: ./eval_results/lego)"
    exit 1
fi

echo "=================================================="
echo "LEGO Manipulation GR00T Checkpoint Evaluation"
echo "=================================================="
...
cat "${SUMMARY_FILE}"
echo ""
echo "To generate HTML training report:"
echo "  python finetuning/plot_checkpoint_results.py --eval_dir ${OUTPUT_BASE} --checkpoint_dir ${CHECKPOINT_DIR}"

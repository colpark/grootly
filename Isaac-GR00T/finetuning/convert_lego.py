#!/usr/bin/env python3
"""
Convert trossen_lego dataset (Hugging Face) to GR00T format for finetuning.

This script converts the Trossen AI Mobile LEGO manipulation dataset
(LeRobot v2 format from Hugging Face) to GR00T-compatible format with train/test splits.

Dataset characteristics:
- Robot: Trossen AI Mobile (bimanual mobile robot) - same as ball2_groot, plug_stacking
- Source: HuggingFace TrossenRoboticsCommunity/trossen_ai_mobile_lego
- 52 episodes, 29,641 frames
...
# Camera keys (same as ball2_groot, plug_stacking)
CAMERA_KEYS = ["cam_high", "cam_left_wrist", "cam_right_wrist"]

# DOF configuration (same as ball2_groot, plug_stacking)
...
def main():
    parser = argparse.ArgumentParser(description="Convert trossen_lego to GR00T format")
    parser.add_argument(
        "--input_path",
        type=str,
        default="./inhouse/lerobot_dataset/lerobot/recorded_data/trossen_lego",
        help="Path to downloaded trossen_lego dataset",
    )
    parser.add_argument(
        "--output_base",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of training episodes (default: total - test_episodes)",
    )
    parser.add_argument(
        "--test_episodes",
        type=int,
    ...
    parser.add_argument(
        "--log_file",
        type=str,
        default="./conversion_lego_log.txt",
    ...
    print(f"Trossen LEGO GR00T Conversion Log")
    print(f"=" * 60)
    ...
    # Task description
    task_description = "LEGO manipulation"

    # Check input exists
    if not input_path.exists():
        print(f"ERROR: Input path not found: {input_path}")
        print("Please download the dataset first:")
        print("  python scripts/download_hf_dataset.py TrossenRoboticsCommunity/trossen_ai_mobile_lego --name trossen_lego")
        sys.exit(1)

    # Read dataset info
    info_file = input_path / "meta" / "info.json"
    if not info_file.exists():
        print(f"ERROR: info.json not found: {info_file}")
        sys.exit(1)
    ...
    if args.train_episodes:
        ...
        if train_count + test_count > total_episodes:
            print(f"ERROR: train ({train_count}) + test ({test_count}) > total ({total_episodes})")
            sys.exit(1)
    else:
        test_count = args.test_episodes
    ...
    train_output = output_base / "lego_train"
    ...
    for new_idx, orig_idx in enumerate(train_indices):
        print(f"\n[Train {new_idx}] Source episode {orig_idx}")
    ...
    test_output = output_base / "lego_test"
    ...
    for new_idx, orig_idx in enumerate(sorted(test_indices)):
        print(f"\n[Test {new_idx}] Source episode {orig_idx}")
